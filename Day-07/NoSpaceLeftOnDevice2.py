...
current_free_space = total_disk_size - root_node.size
required_free_space = 30000000
additional_space_needed = required_free_space - current_free_space

print( f"current = {current_free_space}, additional space needed = {additional_space_needed}")

current_candidate = Node("dummy", size=30000000)
for one_dir in PreOrderIter(root_node):
    if (not one_dir.is_leaf) and (one_dir.size >= additional_space_needed):
        if one_dir.size < current_candidate.size:
            current_candidate = one_dir

print( f"delete node {current_candidate}" )